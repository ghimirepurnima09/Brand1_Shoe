...
class ProductViewModel(val repo: ProductRepo, val imageRepo: ImageRepo) : ViewModel() {
    ...
    fun uploadImage(context: Context, uri: Uri, callback: (String?) -> Unit) = imageRepo.uploadImage(context, uri, callback)
}